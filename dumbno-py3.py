...
class AristaACLManager:
    ...
    def setup(self):
        ...
        for family in self.acls_by_family:
            for port, acl in list(self.ports.items()):
    ...
    def remove_acls(self, to_remove):
        ...
        for acl, entries in list(to_remove.items()):
    ...
    def remove_expired(self):
        ...
        for acl, entries in list(acls.items()):
...
class ACLSvr:
    ...
    def run(self):
        ...
        while True:
            ...
            record = json.loads(data.decode(chardet.detect(data)["encoding"]))
            record = self.mgr.modify_record(record)
            try:
                self.mgr.add_acl(**record)
                zd = "ok"
                self.sock.sendto(bytes(zd, 'UTF-8'), addr)
